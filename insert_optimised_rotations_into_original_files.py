...
optimised_rotation_filename = os.path.join(
    rotation_data_dir,
    'optimisation',
    'all_rotations_{0}.rot'.format(Optimised_APM.model_name))

# Gather all the original '.rot' files (these are our output files).
original_rotation_filenames = glob.glob(os.path.join(rotation_data_dir, '*.rot'))
...
print "Extracting optimized rotations from: %s" % optimised_rotation_filename
...
for rotation_feature_collection_index, rotation_feature_collection in enumerate(original_rotation_feature_collections):
    ...
    if modified_rotation_feature_collection:
        ...
        output_rotation_filename = ''.join((filename_root, '_', Optimised_APM.model_name, filename_ext))
