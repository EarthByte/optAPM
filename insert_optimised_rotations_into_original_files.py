...
#
# If set to False then the 005-000 optimizations are expected to come from the original rotation features instead.
# This can happen if the original rotation features are the output of a previous optimisation run
# and the 005-000 rotations were not removed (eg, by this script).
# In this case we not using the optimisation output of a new optimisation run, instead
# 'remove_005_000_from_original_rotations' is probably True and we just want to remove 005-000
# from the original rotations (such as cleaning up the model for a paper submission).
insert_optimised_rotations_from_optimisation_run = True
...
if remove_005_000_from_original_rotations:
    if insert_optimised_rotations_from_optimisation_run:
        print "...and removing 005-000 from original rotations"
    else:
        print "Removing 005-000 from original rotations"

if insert_optimised_rotations_from_optimisation_run:
    # Read optimized rotation file.
    # This file is the output of an optimisation run.
    optimised_rotation_features = pygplates.FeatureCollection(optimised_rotation_filename)
else:
    # The optimizations just come from the original rotation features.
    # In this case we not using the optimisation output of a new optimisation run,
    # instead we're probably just removing 005-000 from the original rotations.
    optimised_rotation_features = []
    for feature_collection in original_rotation_feature_collections:
        for feature in feature_collection:
            optimised_rotation_features.append(feature)
...
absolute_plate_motion_rotation_features = []
absolute_plate_motion_rotation_sample_times = []
for rotation_feature in optimised_rotation_features:
    ...
    if total_reconstruction_pole:
        ...
        if fixed_plate_id == 0 and moving_plate_id == 5:
            if absolute_plate_motion_rotation_features:
                if insert_optimised_rotations_from_optimisation_run:
...
if not absolute_plate_motion_rotation_features:
    if insert_optimised_rotations_from_optimisation_run:
        ...
        raise ValueError('Expected at least one 005-000 sequence, but got none, in input rotation files')

# May need to re-sort the absolute rotation sample times if we concatenated more than one 005-000 sequence.
# This can happen if we're using 005-000 from original rotation files (eg, could be in separate 0-250Ma and 250-410Ma files).
absolute_plate_motion_rotation_sample_times.sort()

absolute_plate_motion_rotation_model = pygplates.RotationModel(absolute_plate_motion_rotation_features)
